...
void CPU :: ExLD_A(int info)   // Lda Absolute 
{
	...
	int offset = (info & 12) >> 2;  // gets index of register that will be added to address
...
void CPU :: ExLDA_I(int reg)   // Lda Indirect
{
	...
	if (reg == Y) address = mem.ReadByte(address, cycles);     // In case of indirect index (Y)->  the effective address is the value + y
